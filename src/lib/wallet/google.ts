...
import {
  getWalletPublicBaseUrl,
  isWalletPublicHttpsUrl,
  truncateLoyaltyLabel,
} from "@/lib/wallet/google-public-url";
...
export function getAddToGoogleWalletUrl(
  ...
  /** Google cere HTTPS pentru asset-uri din JWT; http:// poate cauza „Something went wrong”. */
  const logoUri =
  ...
  /** Fără classTemplateInfo.firstTopDetail — acel slot e îngust; imaginea 1032×336 era o „fâșie”. */
  const loyaltyClass = {
    id: classId,
  ...
  const loyaltyObject: Record<string, unknown> = {
    ...
    loyaltyPoints: {
      label: truncateLoyaltyLabel("Ștampile"),
      balance: { int: data.stampCount },
    },
    secondaryLoyaltyPoints: {
      label: truncateLoyaltyLabel("Până la"),
  ...
  /** Banner wide — același raport 1032×336 ca în brand guidelines Google (nu slot barcode). */
  if (includeStampImage) {
    loyaltyObject.heroImage = {
      sourceUri: { uri: stampHeroUrl },
      contentDescription: {
        defaultValue: { language: "en-US", value: "Progres ștampile" },
      },
    };
  }
